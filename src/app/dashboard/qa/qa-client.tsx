...
import type { User, PinnedQA, Document } from '@/lib/types'
import { PortfolioCitationPanel } from '@/components/dashboard/portfolio-citation-panel'
      ...
      <section className="bg-white rounded-[2rem] p-6 lg:p-8 border border-zinc-100 shadow-sm">
        ...
        {suggested.length > 0 && (
          <div className="mt-6 flex flex-col gap-3">
            <p className="text-xs font-medium text-zinc-400">추천 질문 — 추가하고 싶은 항목을 선택하세요</p>
            {suggested.map((s, i) => (
              <div key={i} className="rounded-2xl border-2 border-dashed border-zinc-200 p-4">
                <p className="text-sm font-medium text-zinc-800 mb-1">Q. {s.question}</p>
                <p className="text-sm text-zinc-500 leading-relaxed line-clamp-3 mb-3">A. {s.answer}</p>
      ...
      <section className="bg-white rounded-[2rem] p-6 lg:p-8 border border-zinc-100 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">질문과 답변을 바로 편집할 수 있습니다</p>
          </div>
              ...
              <div key={item.id} className="rounded-2xl border border-zinc-100 bg-zinc-50 p-4">
                <div className="flex flex-col gap-3">
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    />
                    <PortfolioCitationPanel
                      documents={portfolio}
                      onCite={(doc) => handleCite(doc, item.id)}
                    />
                  </label>
